...
<body>
  <div class="container">
    <div class="header">
      <h1>📊 DART 기업이벤트 공시 모니터링</h1>
      <div class="date">기준일: ${dateStr}</div>
    </div>
    <div class="body-content">`;

  if (totalDisclosures > 0) {
    html += `
      <span class="summary-badge badge-found">📢 ${totalDisclosures}건의 기업이벤트 공시 발견</span>`;
    ...
    html += `
      ...
      <div class="no-disclosure">
        <div class="icon">✅</div>
        <div class="text">${dateStr} 기준 대상 기업의<br>기업이벤트 관련 공시가 없습니다.</div>
      </div>`;
  }

  html += `
      <div class="target-list">
        <strong>📋 모니터링 대상 기업</strong>
...
export async function sendEventReport(
  ...
  const subject = hasAny
    ? `[DART] 📢 기업이벤트 공시 ${totalCount}건 감지 (${dateStr})`
    : `[DART] ✅ 기업이벤트 공시 없음 (${dateStr})`;

  const html = buildEmailHtml(results, dateStr);

  try {
    const info = await transporter.sendMail({
      from: `"DART 기업이벤트 모니터링" <${config.email.user}>`,
      to: receivers.join(','),
      subject,
      html,
    });
